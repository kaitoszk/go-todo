...
// Create：追加
func (r *TodoRepository) Create(ctx context.Context, title string) error {
	_, err := r.db.ExecContext(ctx, "INSERT INTO todos (title) VALUES ($1)", title)
	return err
}

// Update：タイトル更新
func (r *TodoRepository) Update(ctx context.Context, title string, id int) error {
	_, err := r.db.ExecContext(ctx, "UPDATE todos SET title = $1 WHERE id = $2", title, id)
	return err
}
